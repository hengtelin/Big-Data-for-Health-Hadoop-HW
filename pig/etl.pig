...
-- ***************************************************************************
eventswithmort = JOIN events by patientid LEFT, mortality by patientid;-- perform join of events and mortality by patientid;

deadevents = Filter eventswithmort by(mortality::patientid is not null);-- detect the events of dead patients and create it of the form (patientid, eventid, value, label, time_difference) where time_difference is the days between index date and each event timestamp
...
aliveevents = Filter eventswithmort by(mortality::patientid is null);-- detect the events of alive patients and create it of the form (patientid, eventid, value, label, time_difference) where time_difference is the days between index date and each event timestamp
...
-- ***************************************************************************
filtdead= FILTER deadevents BY (time_difference<=2000 and time_difference>=0);
filtalive= FILTER aliveevents BY (time_difference<=2000 and time_difference>=0);
filtered =union filtdead, filtalive;
...
-- ***************************************************************************
featuresgrp= group filtered by (patientid, eventid);
...
foreach featuresgrp{
  generate group.$0 as patientid, group.$1 as eventid, COUNT(filtered.$0) as featurevalue;
};
...
foreach featureswithid generate eventid;
...
JOIN featureswithid BY eventid LEFT, all_features BY eventid;
...
-- ***************************************************************************
groupedfeatures_idx= group features by idx;
maxvalues = -- group events by idx and compute the maximum feature value in each group. I t is of the form (idx, maxvalues)
foreach groupedfeatures_idx{
  generate group as idx, MAX(features.featurevalue) as maxvalues;
};

normalized = -- join features and maxvalues by idx
join features by idx, maxvalues by idx;
features = -- compute the final set of normalized features of the form (patientid, idx, normalizedfeaturevalue)
foreach normalized generate features::patientid as patientid, features::idx as idx, ((float)features::featurevalue/maxvalues::maxvalues) as normalizedfeaturevalue;
--TEST-5
features = ORDER features BY patientid, idx;
...
--      3,1
-- ***************************************************************************
patientlist= FOREACH eventswithmort GENERATE events::patientid as patientid, (mortality::patientid is null?0:1);

labels = -- create it of the form (patientid, label) for dead and alive patients
distinct  patientlist;
